...
これは，オプティカルフローの正解ラベルをピクセル単位で大量に集めることが困難であるためである．
SNNを用いた手法は，ANNと同等の水準を達成したことが示されているが，現状入手可能なニューロモルフィック
チップ上に実装するにはネットワークのニューロン数が多すぎるという課題があり，実際の視覚処理に組み込もう
と考える場合は，精度を維持しながらネットワークのサイズを削減する工夫が必要となる．
...
視覚処理を行うネットワークでは，密なオプティカルフローを得るために以下の式が用いられた．
...
シンプルであることが特徴である\cite{schulman2017proximal}．PPOでは，目的関数として以下の式\ref{equ:ppo_objective}で表される
クリップ付き代理目的関数が用いられる．
...
低レベルコントローラーが組み合わされている(\ref{fig:bio_training})．高速で移動するゲートを通過するタスクにおいて，SNNエージェントは
...
シグモイド代理勾配関数を用いて学習が行われた．次に，報酬関数には以下の式\ref{equ:reward_function}が用いられた．
...
以下の式\ref{equ:position_reward}で定義される．
...
ついて，SNNはSOTAであるANNの平均成功率である$83\%$を上回る$85\%$を達成した(\ref{fig:bio_performance}(b))．
特に，ゲート間距離が$0.5m$の場合と初期速度が高い場合において，ANNは成功率が大きく低下したが，SNNは安定した
成功率を維持した(\ref{fig:bio_performance}(c))．これは，SNNの方が急速な状態変化への適応性に優れていることを
示している．この理由として，SNNのニューロンが膜電位として時間情報を保持できるため，過去の状態に基づいて現在の
行動を調整できることが挙げられる．次に，完了時間の比較において，ゲート間隔が長くなるに連れて，SNNはANNよりも
短い完了時間であることが示された(\ref{fig:bio_performance}(a))．これについて，SNNはANNよりも軌道全体で
...
この内，$15~20\%$が活性化されており，23ニューロンの入力層と4ニューロンの出力層を含めた場合の実際の演算数
...
が重要であると考えられる．一方，進化アルゴリズムは
強化学習は，未知の環境への適応性が高いことが特徴であるが，この特徴を
最大限に活用するためには，適切な報酬関数の設計が重要である．
今後の研究では，火星表面の風による外乱への堅牢性を獲得するために，外乱を含むシミュレーション環境での
強化学習の実施や，イベントカメラによる視覚処理と強化学習を組み合わせた制御モデルを構築することを
目指したいと考えている．
